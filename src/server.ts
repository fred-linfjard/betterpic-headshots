...
const headshotResults: Record<string, { status: string; image_urls?: string[]; submitted_at: string }> = {};
...
// Register a pending headshot job
app.post('/results/register', express.json(), (req: any, res: any) => {
  const { headshots_id } = req.body;
  if (headshots_id && !headshotResults[headshots_id]) {
    headshotResults[headshots_id] = { status: 'processing', submitted_at: new Date().toISOString() };
  }
  const resultsUrl = `https://betterpic-headshots-production.up.railway.app/results/${headshots_id}`;
  res.json({ url: resultsUrl });
});

// Webhook callback from Runflow when headshots are ready
app.post('/webhook/headshots', express.json(), (req: any, res: any) => {
  const { event, headshots_id, image_urls } = req.body;
  if (event === 'headshots.completed' && headshots_id) {
    headshotResults[headshots_id] = {
      status: 'completed',
      image_urls,
      submitted_at: headshotResults[headshots_id]?.submitted_at || new Date().toISOString()
    };
  }
  res.sendStatus(200);
});
...
// Results page
app.get('/results/:id', (req: any, res: any) => {
  const { id } = req.params;
  const result = headshotResults[id];
  const isReady = result?.status === 'completed';
...
<title>BetterPic Headshots</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header h1 { font-size: 20px; font-weight: 700; color: #7A52F4; }
  .header p { font-size: 13px; color: #6b7280; margin-top: 4px; }
  .container { max-width: 1200px; margin: 0 auto; padding: 32px 24px; }
  .status-card { border-radius: 12px; padding: 28px; margin-bottom: 32px; border: 1px solid #e5e7eb; text-align: center; background: white; }
  .status-card.processing { border-color: #f59e0b; background: #fffbeb; }
  .status-card.ready { border-color: #22c55e; background: #f0fdf4; }
  .status-card.notfound { border-color: #ef4444; background: #fef2f2; }
  .status-icon { font-size: 52px; margin-bottom: 16px; }
  .status-card h2 { font-size: 22px; font-weight: 700; margin-bottom: 8px; }
  .status-card p { color: #6b7280; font-size: 15px; margin-bottom: 6px; }
  .meta { font-size: 12px; color: #9ca3af; }
  .refresh-btn { display: inline-block; margin-top: 16px; background: #f59e0b; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px; cursor: pointer; border: none; }
  .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 12px; }
  .grid a img { width: 100%; aspect-ratio: 1; object-fit: cover; border-radius: 10px; border: 1px solid #e5e7eb; transition: transform 0.2s; display: block; }
  .grid a img:hover { transform: scale(1.04); box-shadow: 0 8px 24px rgba(0,0,0,0.15); }
  .count { font-size: 14px; color: #6b7280; margin-bottom: 16px; }
  .warning { font-size: 13px; color: #d97706; margin-top: 24px; background: #fffbeb; padding: 10px 16px; border-radius: 8px; border: 1px solid #f59e0b; }
</style>
</head>
<body>
<div class="header">
...
<div class="container">
  ${!result ? `
  <div class="status-card notfound">
    <div class="status-icon">❓</div>
    <h2>Job Not Found</h2>
    <p>This headshot job ID doesn't exist or may have expired.</p>
    <p class="meta">Note: Results are stored in memory and reset when the server restarts.</p>
  </div>
  ` : !isReady ? `
  <div class="status-card processing">
    <div class="status-icon">⏳</div>
    <h2>Processing...</h2>
    <p>Your 60 AI headshots are being generated by BetterPic.</p>
    <p>Estimated time: approximately 2 hours.</p>
    <p class="meta">Submitted: ${new Date(result.submitted_at).toLocaleString()}</p>
  ...
  <div class="status-card ready">
    ...
    <p>60 professional AI headshots generated successfully.</p>
    <p class="meta">Completed: ${new Date(result.submitted_at).toLocaleString()}</p>
  </div>
  <p class="count">${images.length} headshots · Click any image to open full size</p>
  <div class="grid">
    ${images.map((url: string) => `<a href="${url}" target="_blank"><img src="${url}" alt="Headshot" loading="lazy"></a>`).join('')}
  </div>
  <p class="warning">⚠️ These images expire after 24 hours. Right-click and save the ones you want to keep.</p>
  `}
</div>
</body>
</html>`);
    ...
    case 'tools/call':
      if (params.name === 'score-images') {
        try {
          ...
          if (!key) throw new Error("No API key. Please open widget settings (⚙️) and enter your Runflow API key.");
        ...
        try {
          ...
          if (!key) throw new Error("No API key. Please open widget settings.");
          ...
          const data = await response.json() as { headshots_id: string };
        ...
        try {
          ...
          if (callbackUrl) savedCallbackUrl = callbackUrl;
